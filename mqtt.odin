...
pal_time_t        :: libc.time_t
pal_socket_handle :: i64
...
fixed_header :: bit_field u32 {
	/** The type of packet. */
	control_type: ControlPacketType | 4,
...
response_connack :: struct #packed {
	/** 
...
response_pubrel :: struct {
	/** @brief The published messages packet ID. */
	packet_id : u16
};

SubackReturnCodes :: enum {
	SUCCESS_MAX_QOS_0 = 0,
	SUCCESS_MAX_QOS_1 = 1,
	SUCCESS_MAX_QOS_2 = 2,
	FAILURE           = 128
...
response_unsuback  :: struct{
	/** @brief The published messages packet ID. */
	packet_id: u16,
};

response_puback :: struct{
	/** @brief The published messages packet ID. */
	packet_id: u16,
};
response_pubrec:: struct{
	/** @brief The published messages packet ID. */
	packet_id: u16,
};

response_pubcomp:: struct{
	/** @brief The published messages packet ID. */
	packet_id: u16,
};


response_pingresp  :: struct{
  dummy: i32,
};
...
response :: struct{
	/** @brief The FixedHeader of the deserialized packet. */
	...
	decoded : struct #raw_union{
		connack : response_connack,
		publish : response_publish,
		puback  : response_puback,
		pubrec  : response_pubrec,
		pubrel  : response_pubrel,
		pubcomp : response_pubcomp,
		suback  : response_suback,
		unsuback: response_unsuback,
		pingresp: response_pingresp 
	}
};
...
SUBSCRIBE_REQUEST_MAX_NUM_TOPICS :: 8
UNSUBSCRIBE_REQUEST_MAX_NUM_TOPICS :: 8

QueuedMessageState :: enum {
	UNSENT,
	AWAITING_ACK,
	COMPLETE
};
...
queued_message :: struct{
	 ...
	 *       the QUEUED_AWAITING_ACK \c state.
	 */
	time_sent: pal_time_t,
...
message_queue :: struct {
	 ...
	 */
	queue_tail: ^queued_message,
};


Client :: struct {
	/** @brief The socket connecting to the MQTT broker. */
	socketfd: pal_socket_handle,
	...
	*/
	 ...
	 *       need.
	 */
	publish_response_callback : proc "c" (state: ^rawptr, publish: ^response_publish),
	 ...
	 * time.
	 */
	 inspector_callback : proc(client: ^Client) -> Errors,
	 ...
	 */
	 reconnect_callback : proc(client: ^Client, data: ^rawptr),
